...
for id in data_dict.keys():
    ...
    try:
        precision = format(
            precision_score(data_dict[id]["real_TF"], data_dict[id]["predict_TF"], zero_division=np.nan), ".2f"
        )
    ...
    try:
        recall = format(
            recall_score(data_dict[id]["real_TF"], data_dict[id]["predict_TF"], zero_division=np.nan), ".2f"
        )
    except ZeroDivisionError as e:
        recall = "Err"
