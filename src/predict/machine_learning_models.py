...
def predict(explanatory_variable, label, project_name: str, model_name: str):
    ...
    X_train.to_csv(f"./results/conventional_method/{project_name}_train.csv")
...
def create_model(cnum: int, model_name: str):
    ...
    for i in range(cnum):
        try:
            model_dict["cluster_" + str(i)].fit(
                df_marge[df_marge["Cluster_num"] == i].drop(["Project_name", "Cluster_num", "AnsTF"], axis=1),
